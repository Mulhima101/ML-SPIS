...
export const updateStudentProfile = async (studentId, profileData) => {
  try {
    const response = await axios.put(`${API_URL}/students/${studentId}/profile`, profileData);
    return response.data;
  } catch (error) {
...
export const getKnowledgeLevels = async (studentId) => {
  try {
    const response = await axios.get(`${API_URL}/students/${studentId}/knowledge`);
    return response.data;
  } catch (error) {
    throw error.response ? error.response.data : new Error('Failed to get knowledge levels');
  }
};

export const getPersonalizedGuidance = async (studentId) => {
  try {
    const response = await axios.get(`${API_URL}/students/${studentId}/guidance`);
    return response.data;
  } catch (error) {
    throw error.response ? error.response.data : new Error('Failed to get guidance');
  }
};
...
export const getStudentQuizzes = async (studentId) => {
  try {
    const response = await axios.get(`${API_URL}/students/${studentId}/quizzes`);
    return response.data;
  } catch (error) {
        ...
        recommendations: [
          {
            type: 'resource',
            title: 'Fundamentals',
            description: 'Review the basic concepts of Network Engineering to build a solid foundation.',
            link: '/resources/fundamentals'
          },
          {
            type: 'practice',
            title: 'Basic Practice',
            description: 'Complete basic exercises to reinforce your understanding of Network Engineering.',
            link: '/practice/basic'
          }
        ]
      },
        ...
        recommendations: [
          {
            ...
            description: 'Consider sharing your knowledge of SDLC with peers or in online forums.',
            link: '/community'
          }
        ]
      }
